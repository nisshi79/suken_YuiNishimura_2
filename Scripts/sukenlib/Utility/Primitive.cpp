...
bool suken::HitRectRect(const Rect& r1, const Rect& r2) {
	return r1.right > r2.left && r2.right > r1.left && r1.bottom > r2.top && r2.bottom > r1.top;
}

bool suken::HitRectRectF(const RectF& r1, const RectF& r2) {
	return r1.right > r2.left && r2.right > r1.left && r1.bottom > r2.top && r2.bottom > r1.top;
}

void suken::HitMRectRect(int& x, int& y, int& vx, int& vy, const Rect& r1, const Rect& r2, const RectFlag& enable, RectFlag& flag) {//ë¨ìxÇÕë´ÇµçáÇÌÇπÇΩå„ÇëzíË
	Int2 vec = { x,y };
...
void suken::HitMRectRectF(float& x, float& y, float& vx, float& vy, const RectF& r1, const RectF& r2, const RectFlag& enable, RectFlag& flag) {
	Vector2D vec = { x,y };

	if (HitRectRectF(r1 + vec, r2)) {
		vec.x -= vx;
		vec.y -= vy;
		RectF l = r1 + vec;
